...
from Base import DrawConfig, Orientation, Alignment
...
class AutoGeneratedAxisDataSource(AxisDataSource):
    ...
    def append_by_index_sequence(self, xs: List[float], align: "Alignment" = Alignment.BEFORE):
        for x in xs:
    ...
    def draw_x(self, config: "DrawConfig",
        ...
        for text_info in self.data_source:  # type: TextLabelInfo
            ...
            if align is Alignment.BEFORE:
                pos = QRectF(ui_x - text_width,
                             text_top,
                             rect.width(),
                             rect.height(),
                             )
            elif align is Alignment.MID:
                pos = QRectF(ui_x - text_width / 2,
                             text_top,
                             rect.width(),
                             rect.height(),
                             )
...
class BarAxis(ValueAxis):
    ...
    def prepare_draw_grids(self, config: "DrawConfig", painter: "QPainter") -> None:
        seq = ValueSequenceGenerator(self, self.label_count+1).prepare(config, painter)
        ds = self.grid_drawer.data_source
        ds.clear()
