...
from DataSource import CandleDataSource, DataSource
...
class AutoGeneratedAxisDataSource(AxisDataSource):
    ...
    @abstractmethod
    def append_by_index(self, x: float, align: "Alignment" = Alignment.BEGIN):
        raise NotImplementedError()
    ...
    def append_by_index_sequence(self, xs: List[float], align: "Alignment" = Alignment.BEGIN):
        for x in xs:
...
class AxisBase(ABC):
    ...
    don't inherit this class directly, inherit AxisXBase or AxisYBase.
    """
    ...
    def __init__(self,
        ...
        self.grid_drawer: Optional["GridDrawer"] = None
...
class LineGridDrawer(GridDrawer, ABC):
    ...
    def draw_y(self, config: "DrawConfig",
               painter: QPainter):
        painter.setPen(QPen(QColor(self.grid_color)))
        drawing_cache = config.drawing_cache
...
TextFlag = Qt.AlignLeft | Qt.AlignTop
...
class DateTimeDataSource(AxisDataSource):
    ...
    def append_by_sequence(self, xs: List[float], align: "Alignment", dts: List[datetime]):
...
class CandleLabelDataSource(AutoGeneratedAxisDataSource, DateTimeDataSource):
    ...
    def __init__(self,
                 orientation: "Orientation",
                 candle_data_source: CandleDataSource,
                 format: str = None,
                 ):
        super().__init__()
    ...
    def append_by_index(self, x: int, align: "Alignment" = Alignment.BEGIN):
        data = self.candle_data_source[x]
        self.append(TextLabelInfo(x, self.format % data.datatime, align))
...
class TextLabelDrawer(LabelDrawer, ABC):
    ...
    def draw_x(self, config: "DrawConfig",
               painter: QPainter):
        drawing_cache = config.drawing_cache
        text_top = drawing_cache.plot_area.bottom() + 1 + self.axis.label_spacing_to_plot_area

        for text_info in self.data_source:
            ui_x = drawing_cache.drawer_x_to_ui(text_info.value)
            text = text_info.text
            rect = painter.boundingRect(0, 0, 1000, 1000, text_info.flag, text)
...
class ValueAxis(AxisBase):
    ...
    def __init__(self, orientation: "Orientation"):
        ...
        self.label_drawer.data_source = ValueLabelDataSource()

    def prepare_draw_grids(self, config: "DrawConfig", painter: "QPainter") -> None:
        seq = ValueSequenceGenerator(self, self.label_count).prepare(config, painter)
        self.grid_drawer.data_source.append_by_index_sequence(seq, Alignment.MID)

    def prepare_draw_labels(self, config: "DrawConfig", painter: "QPainter") -> None:
        seq = ValueSequenceGenerator(self, self.label_count).prepare(config, painter)
        self.label_drawer.data_source.append_by_index_sequence(seq, Alignment.MID)
...
#         return self.data_source[int(value)].datetime.strftime(self.format)
